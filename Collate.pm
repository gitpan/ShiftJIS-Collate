...
sub _replaced($$)
{
  ...
  my $n = pack('n', $p);
  return 
    $c eq '[' ? "\x79".chr($d == 6 ? 0xd6 : 0x40 + $d)."\x34\x1a\x11\x03" :
    $c eq '°'  ? "\x79".chr($d == 6 ? 0xd6 : 0x40 + $d)."\x34\x1a\x11\x04" :
    $c eq 'T'                         ? "$n\x34\x1c\x10\x03" :
    $c eq 'R'                         ? "$n\x34\x1c\x11\x03" :
    $c eq 'U' && _hasDakuHiragana($n) ? "$n\x35\x1c\x10\x03" :
    $c eq 'S' && _hasDakuKatakana($n) ? "$n\x35\x1c\x11\x03" :
    undef;
}
