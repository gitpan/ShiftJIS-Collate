...
'ÅX' => "\x7a\x42\x32\x14\x0a\x01",
'ÅW' => "\x7a\x43\x32\x14\x0a\x01",
...
E.g., an element of @source is probably C<['ì˙ñ{åÍ', 'Ç…ÇŸÇÒÇ≤']>;
Its opposite, C<['Ç…ÇŸÇÒÇ≤', 'ì˙ñ{åÍ']>, is also allowed, though.

B<Yomi-hyoki shogo> is carried out through two comparison stages.

E.g., sort these strings by 'Yomi-hyoki shogo'.

C<['âiìc', 'Ç»Ç™ÇΩ']>, C<['è¨éR', 'Ç®Ç‚Ç‹']>, C<['í∑ìc', 'Ç®Ç≥Çæ']>, C<['í∑ìc', 'Ç»Ç™ÇΩ']>, C<['è¨éR', 'Ç±Ç‚Ç‹']>.

First, order by reading
(C<'Ç®Ç≥Çæ'> E<lt> C<'Ç®Ç‚Ç‹'> E<lt> 'Ç±Ç‚Ç‹' E<lt> 'Ç»Ç™ÇΩ');
next, order by spelling among strings having the same reading (C<'âiìc'> E<lt> C<'í∑ìc'> where both are read as C<'Ç»Ç™ÇΩ'>).

The result should be C<['í∑ìc', 'Ç®Ç≥Çæ']> E<lt> C<['è¨éR', 'Ç®Ç‚Ç‹']> E<lt> C<['è¨éR', 'Ç±Ç‚Ç‹']> E<lt> C<['âiìc', 'Ç»Ç™ÇΩ']> E<lt> C<['í∑ìc', 'Ç»Ç™ÇΩ']>.
...
This ordered list is an example of the result of C<"kan'i-daihyo-yomi shogo">.
  ...
  ['ÇSéüå≥', 'ÇÊÇ∂Ç∞ÇÒ'],
  ['ÇUñ ëÃ', 'ÇÎÇ≠ÇﬂÇÒÇΩÇ¢'],
  ['ÉøïˆâÛ', 'ÉAÉãÉtÉ@ÇŸÇ§Ç©Ç¢'],
  ['É°ä÷êî', 'ÉKÉìÉ}Ç©ÇÒÇ∑Ç§'],
  ['É¿ê¸',   'ÉxÅ[É^ÇπÇÒ'],
  ['Çpíl',   'ÉLÉÖÅ[Çø'],
  ['ÇiÇhÇr', 'Ç∂Ç∑'],
  ['Perl',   'ÉpÅ[Éã'],
...
(1) Compare the character class of the first character of the spell.
  ...
  Digit class ('ÇSñ ëÃ') < Greek class ('ÉøïˆâÛ') < Latin class ('ÇiÇhÇr') < Kanji class ('ä÷ìå').

(2) Compare the first character of the reading.

  e.g. 'ÇµÇﬂÇÒÇΩÇ¢' < 'Ç…ÇµÇÂÇ≠ÇπÇ¢' < 'ÇÊÇ∂Ç∞ÇÒ' < 'ÇÎÇ≠ÇﬂÇÒÇΩÇ¢'.

(3) Compare the first character of the spell.

  e.g. ('âÕêº','âÕìc',etc.) < ('äpìc','äpìc') < ('ä÷ìå');

       ('ëÚìá','ëÚìà','ëÚìc') < ('‡Vìá','‡Vìà','‡Vìc').

(4) Compare the whole string of the reading.

  e.g. ['âÕêº', 'Ç©Ç≥Ç¢'] < ['âÕçá', 'Ç©ÇÌÇ¢'] < ['âÕìc', 'Ç©ÇÌÇæ'];

       ['äpìc', 'Ç©Ç≠ÇΩ'] < ['äpìc', 'Ç©Ç«ÇΩ'].

(5) Compare the whole string of the spell.

  e.g. ['ëÚìá', 'Ç≥ÇÌÇµÇ‹'] < ['ëÚìà', 'Ç≥ÇÌÇµÇ‹'] < ['ëÚìc', 'Ç≥ÇÌÇæ'].
...
=head2 Replacement of PROLONGED SOUND MARKs and ITERATION MARKs
   ...
   Character  SJIS    UCS     Name
     ...
     'Å['    0x815B  U+30FC  KATAKANA-HIRAGANA PROLONGED SOUND MARK
     '∞'     0xB0    U+FF70  HALFWIDTH KATAKANA-HIRAGANA PROLONGED SOUND MARK
     'ÅT'    0x8154  U+309D  HIRAGANA ITERATION MARK
     'ÅU'    0x8155  U+309E  HIRAGANA VOICED ITERATION MARK
     'ÅR'    0x8152  U+30FD  KATAKANA ITERATION MARK
     'ÅS'    0x8153  U+30FE  KATAKANA VOICED ITERATION MARK
...
C<PROLONGED SOUND MARK replaced by KATAKANA A>, and C<KATAKANA RU>.
  ...
  e.g.,
   ...
   'ÉpÅ[Éã' is converted to 'ÉpÉAÉã' by replacement;
     ...
     condary equal to 'ÇœÇ†ÇÈ', greater than 'ÇÕÇ†ÇÈ';
     tertiary equal to 'ÇœÅ[ÇÈ', lesser than 'ÉpÉAÉã';
     and quartenary greater than 'ÇœÅ[ÇÈ'.
